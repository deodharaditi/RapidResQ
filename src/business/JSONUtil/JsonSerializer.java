...
public class JsonSerializer {
    ...
    /**
     * Constructs a JsonSerializer instance that can serialize domain model
    ...
    /**
     * Sets up the XStream aliases for model classes to make the output JSON
     * more human-friendly.
     */
    ...
    /**
     * Formats the input JSON string to make it easier for humans to read. Based
     * on stackoverflow.com/a/46519130.<br>
     *
     */
    private static String formatJsonString(final String json) {
        ...
        for (int i = 0, indent = 0; i < chars.length; i++) {
            ...
            if (!beginQuotes) {
                result += switch (c) {
                    case '{', '[' ->
                        c + newline + String.format("%" + (indent += INDENT_WIDTH) + "s", "");
                    case '}', ']' ->
                        newline + ((indent -= INDENT_WIDTH) > 0 ? String.format("%" + indent + "s", "") : "") + c;
                    case ':' ->
                        c + " ";
                    case ',' ->
                        c + newline + (indent > 0 ? String.format("%" + indent + "s", "") : "");
                    default ->
                        "";
                };
